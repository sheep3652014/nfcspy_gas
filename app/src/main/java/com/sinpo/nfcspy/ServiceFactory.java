...
final class ServiceFactory {
	...
	static void setTag2Server(Context ctx, Intent nfc) {
		///----------------------> anaysis
		System.out.println("----------> nfc intent " + nfc.toString()+"___"+nfc.getExtras().toString()+"__");
		///----------------------< anaysis
		final Intent i = newServerIntent(ctx, ACTION_NFC_ATTACH);
		ctx.startService(i.replaceExtras(nfc)); // replace the newServerIntent Extras data as nfc intent and start ServiceFull
	}
	...
	/**
	 * 有HCE模式，新建ServiceFull intent并返回，
	 * 没有新建ServiceLite intent并返回
	 * @param ctx
	 * @param action
	 * @return
	 */
	private static Intent newServerIntent(Context ctx, String action) {
		...
		if (NfcManager.hasHCE()) {
			System.out.println("---> anaysis  hashce");
			try {
		...
		System.out.println("----> anaysis " + clazz.toString()+"___" + clazz.getName());
