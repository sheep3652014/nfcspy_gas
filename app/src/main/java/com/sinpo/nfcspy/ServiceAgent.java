...
final class ServiceAgent extends BroadcastReceiver implements ChannelListener,
    ...
    private void onStartCommand(Intent intent) { //回传wifi p2p状态信息之activity
        ...
        if ((outbox = messenger) != null) {
            ...
            new Wifip2pRequestConnectionInfo(this).execute(p2p); /// 更具wifi p2p连接初始化状态发送信息至activity,并保持相关链接信息
        }
    }
